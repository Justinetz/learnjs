const dec2TestData = `9-12 q: qqqxhnhdmqqqqjz
...
var splitToRows = function (all) {
	const rows = all.split('\n');
	return rows;
};

const rows = splitToRows(dec2TestData);
let valid = 0;
let posValid = 0;
rows.forEach(r => {
	const parts = r.split(':');
	const rule = parts[0].split(' ');
	const counts = rule[0].trim().split('-');
	const symbol = rule[1].trim();
	const passw = parts[1].trim();
	const min = +counts[0];
	const max = +counts[1];
	console.log('find ' + symbol + ' in ' + passw);
	const counter = passw.split(symbol).length - 1;
	if (counter >= min && counter <= max) {
		valid++
	}
	if ((passw[min-1] === symbol && passw[max-1] !== symbol) || (passw[min-1] !== symbol && passw[max-1] === symbol)) {
		posValid++;
	}
});
console.log(valid, posValid);